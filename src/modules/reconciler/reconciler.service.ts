...
export class ReconcilerService {
  ...
  /** Cron entry: reconcile every open pending payment. */
  async pollOpenPayments(): Promise<{ processed: number; outcomes: Record<string, number> }> {
    ...
    this.logger.log(
      { processed: open.length, outcomes },
      'poll-open-payments finished',
    );
    return { processed: open.length, outcomes };
  }
  ...
  async reconcilePayment(payment: Payment): Promise<ReconcileOutcome> {
    ...
    const result = this.verifier.verify(payment, records, knownHashes);
